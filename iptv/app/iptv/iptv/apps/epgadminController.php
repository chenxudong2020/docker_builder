...
if (isset($_POST['bindchannel'])) {
    ...
    if (isset($_POST['id'])) {
        ...
        for($i=0;$i<count($result);$i++){ //根据每个搜索关键词对应的名称存入$list数组
            foreach ($clist as $cname => $channel) {
                if (strstr($channel['name'], $result[$i]) !== false) {
    ...
    if (!mysqli_num_rows($result)) {
        ...
        for($i=0;$i<count($result_remarks);$i++){ //根据每个搜索关键词对应的名称存入$list数组
            foreach ($clist as $cname => $channel) {
                if (strstr($channel['name'], $result_remarks[$i]) !== false) {
                    $list[$cname] = $channel['name'];
                } 
            } 
        }
